...
#### 2. Enhanced Overview Dashboard (نظرة عامة)
- **Sales KPIs**: The top metrics cards now explicitly display exact consumer counts across all 4 risk tiers alongside pipeline metrics.
- **Top Deals at Risk (أهم الصفقات التي تتطلب تدخل سريع)**: Added a new live widget at the bottom of the overview that lists the highest-revenue accounts in critical danger, providing a direct "Review" button.
- **VIP Accounts Ratio (نسبة عملاء VIP)**: Added a new doughnut chart breaking down the pipeline ratio of strategic VIP accounts versus standard accounts.
- **Explicit Numerical Breakdowns**: The Risk Distribution chart now features a clear, color-coded grid underneath it showing the exact number of accounts in each risk tier, eliminating the need to hover over the chart.

#### 3. Action-Oriented Customer Table (جدول العملاء)
